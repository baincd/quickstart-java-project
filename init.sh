...
while true; do
    read -p "Keep quickstart-typescript-project in $ARTIFACT_ID git history? ([Y]/n): " yn
    case $yn in
...
git commit -a -m "Initialize $ARTIFACT_ID POM" || true
...
git add -A . && git commit -m "Initialize $ARTIFACT_ID Java classes" || true
